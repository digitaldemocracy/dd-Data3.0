...
def add_sponsor(dd_cursor, pid, bid, vid, contribution):
    ...
    if dd_cursor.rowcount == 0:
        ...
        try:
            ...
            logger.exception(format_logger_message('Insert failed for BillSponsors', (QI_BILLSPONSORS % (pid, bid, vid, contribution))))
        ...
        if version[contribution]['size'] > 0 and check_bid_db(bill['bid'], dddb) and vid_check:
            for sponsors in version[contribution]['items']:
                try:
                    ...
                    logger.exception('Problem with name ' + full_name)
...
def get_pid_db(first, last, dddb):
    ...
    if dddb.rowcount >= 1:
        ...
        logger.exception('Person not found ' + last + ', ' + first)
        return None
        ...
        for contribution in ('coSponsors', 'multiSponsors'):
            ...
            if dddb.rowcount == 0:
                try:
                    ...
                    logger.exception(format_logger_message('Insert failed for BillSponsorRoll', (QI_BILLSPONSORROLLS % (contribution,))))
