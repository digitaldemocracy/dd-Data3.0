...
def insert_lobbyistEmployer_db(dddb, lobby):
    ...
    if query is None:
        try:
            ...
            logger.exception(format_logger_message('Insert failed for LobbyistEmployer',
                                                   (QI_LOBBYISTEMPLOYER % (lobby['client_oid'],
                                                                           lobby['client_name'], 'NY'))))
    ...
    if query is None:
        try:
            ...
            logger.exception(format_logger_message('Insert failed for Organizations',
                                                   (QI_ORGANIZATIONS % (lobby['client_name'],
                                                                        lobby['client_city'],
                                                                        lobby['client_state']))))
...
def insert_direct_employment_db(dddb, lobbyist):
    ...
    for person in lobbyist['lobbyists']:
        ...
        try:
            ...
            dddb.execute(QS_LOBBYISTDIRECTEMPLOYMENT,
                         (pid, lobbyist['client_oid'], lobbyist['ls_beg_yr'], lobbyist['ls_end_yr'], 'NY'))
            if dddb.rowcount == 0:
                rpt_period = (lobbyist['rpt_period'] + 1) * 2 + 1
                if rpt_period == 13:
                    rpt_period = 1
                rpt_date = date(lobbyist['rpt_year'], rpt_period, 1)
                dddb.execute(QI_LOBBYISTDIRECTEMPLOYMENT, (
                pid, lobbyist['client_oid'], rpt_date, lobbyist['ls_beg_yr'], lobbyist['ls_end_yr'], 'NY'))
                LDE_INSERT += dddb.rowcount
        except MySQLdb.Error:
            logger.exception(format_logger_message('Insert failed for LobbyistDirectEmployment',
                                                   (QI_LOBBYISTDIRECTEMPLOYMENT %
                                                    (
                                                        pid, lobbyist['client_oid'], rpt_date,
                                                        lobbyist['ls_beg_yr'], lobbyist['ls_end_yr'], 'NY')
                                                    )))
...
def insert_lobbying_contracts_db(dddb, lobbyist):
    ...
    dddb.execute(QS_LOBBYINGCONTRACTS,
                 (lobbyist['filer_id'], lobbyist_employer, lobbyist['ls_beg_yr'], lobbyist['ls_end_yr'], 'NY'))
    ...
    if dddb.rowcount == 0:
        ...
        rpt_date = date(lobbyist['rpt_year'], rpt_period, 1)
        try:
            dddb.execute(QI_LOBBYINGCONTRACTS, (
            lobbyist['filer_id'], lobbyist_employer, rpt_date, lobbyist['ls_beg_yr'], lobbyist['ls_end_yr'], 'NY'))
            LC_INSERT += dddb.rowcount
        except MySQLdb.Error:
            logger.exception(format_logger_message('Insert failed for LobbyingContracts',
                                                   (QI_LOBBYINGCONTRACTS % (
                                                       lobbyist['filer_id'], lobbyist_employer, rpt_date,
                                                       lobbyist['ls_beg_yr'], lobbyist['ls_end_yr'], 'NY'))))
...
def insert_lobbyingfirm_db(dddb, lobbyist):
    ...
    if dddb.rowcount == 0:
        try:
            ...
            logger.exception(format_logger_message('Insert failed for LobbyingFirm', (QI_LOBBYINGFIRM % lobbyist)))
    ...
    if dddb.rowcount == 0:
        try:
            ...
            logger.exception(format_logger_message('Insert failed for LobbyingFirmState',
                                                   (QI_LOBBYINGFIRMSTATE % (lobbyist['filer_id'],
                                                                            lobbyist['filer_naml'], 'NY'))))
