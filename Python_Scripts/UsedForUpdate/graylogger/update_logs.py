...
"""
      table_name_every_even.append(tables.split(":"))
    for i in range (0, len(table_couple) + 1, 2):
      table_couple.append(table_name_every_even[i], table_name_every_even[i + 1])
    for tables in table_couple:
      middb_cursor.execute('''SELECT tid FROM DD_Table WHERE name=%s''', (table_couple[0],))
      tid_array.append(middb_cursor.fetchone()[0], table_couple[1])
  return tid_array
"""
