...
def addBill(cursor, bid, type, number, state, status, house, session):
  ...
  if(cursor.rowcount == 0):
    ...
    cursor.execute(insert_bill_stmt, (bid, type, number, state, status, house, session, us_state))
...
def addBillVersion(cursor, vid, bid, date, state, subject, appropriation, substantive_changes):
  ...
  if(cursor.rowcount == 0):
    ...
    cursor.execute(insert_billversion_stmt, (vid, bid, date, state, subject, appropriation, substantive_changes, us_state))

# Finds the state of the bill
# Used as a helper for finding BillVersions
def findState(cursor, bid):
  select_stmt = "SELECT billState from Bill where bid = %(bid)s"
