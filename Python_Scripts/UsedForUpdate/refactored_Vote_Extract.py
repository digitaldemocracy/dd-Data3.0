...
TODO: FIX
'''
def get_person(cursor, name, floor):
	pid = -1
	#filer_naml = clean_name(filer_naml)
  cleaned_name = clean_name(name)
	name_words = cleaned_name.split(' ')
	floor = 'Assembly' if floor == 'AFLOOR' else 'Senate'
	filer_namf = ''
	if len(name_words) > 1:
		filer_naml = name_words[-1]
		filer_namf = name_words[0]
		cursor.execute('''SELECT pid, last, first 
                      FROM Person 
                      WHERE last = %s 
                       AND first = %s 
                       AND pid < 130 
                      ORDER BY Person.pid
                   ''', (filer_naml, filer_namf))
	else:
		cursor.execute('''SELECT pid, last, first 
                      FROM Person 
                      WHERE last = %s 
	...
	if cursor.rowcount == 1:
		...
		a = []
		for j in xrange(0, cursor.rowcount):
		...
		select_pid = '''SELECT pid, last, first 
                    FROM Person 
		...
		if cursor.rowcount > 0:
			...
			print('Could not find %s' % filer_naml)
	return pid
