...
def main():
    ...
    write_tbl(verb_df, conn, 'LegParticipationVerbal_analyt', by_leg_key, index=False)
    write_tbl(verb_df_com, conn, 'LegParticipationVerbalCom_analyt', by_com_key, index=False)

    stmt_df = calc_avg_statement_len(df, by_leg_key)
    stmt_df_com = calc_avg_statement_len(df_com, by_com_key)

    write_tbl(stmt_df, conn, 'LegParticipationStmtLen_analyt', by_leg_key, index=False)
    write_tbl(stmt_df_com, conn, 'LegParticipationStmtLenCom_analyt', by_com_key, index=False)
